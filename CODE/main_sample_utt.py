...
import pytrec_eval
import json
from python_code import conv_measures, eval_collections, utils
import logging
import time
import pandas as pd
...
tstart = time.time()
#topic_evaluator = {t: pytrec_eval.RelevanceEvaluator({t: CAsT.qrels_ts[t]}, {'ndcg_cut'})
...
for runID in CAsT.runs:
    measures_orig[runID] = {t: topic_evaluator[t].evaluate({t: CAsT.runs[runID][t]})[t]['ndcg_cut_3'] for t in
                            CAsT.qrels_ts}
...
print(convs)
logging.info('Generating new convos by randomly pruning original ones')
tstart = time.time()
new_convs = {}
for c in convs:
    new_convs[c] = [convs[c]]
    for _ in range(1000):
        new_c = utils.random_prune(convs[c])
        alreadyIn = False
        for conv in new_convs[c]:
            if utils.compare_trees(new_c, conv):
                alreadyIn = True
                break
        if not alreadyIn:
            new_convs[c].append(new_c)
        if len(new_convs[c]) == 20:
            break

for c in new_convs:
    temp = new_convs[c]
    new_convs[c] = {}
    for e, conv in enumerate(temp):
        new_convs[c][e] = conv
...
for alpha in alphas:
    ...
    for runID in CAsT.runs:
        new_measures[runID] = {}
        for c in new_convs:
            new_measures[runID][c] = {}
            for e in new_convs[c]:
                new_measures[runID][c][e] = conv_measures.recursive_measure(new_convs[c][e], measures[runID][c], alpha=alpha)


    for runID in new_measures:
        for c in new_measures[runID]:
            for csid in new_measures[runID][c]:
                for e, v in enumerate(new_measures[runID][c][csid]):
                    if not np.isnan(v):
                        results.append([runID, c, csid, e, alpha, v])

results = pd.DataFrame(results, columns=["run", "conv", "sampleid", "utt", "alpha", "score"])


results.to_csv("../data/sampled_utts_NDCG_cut_3.csv")
'''
