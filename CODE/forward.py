...
topic_evaluator = {t: pytrec_eval.RelevanceEvaluator({t: CAsT.qrels_ts[t]}, {pytrec_label1})
                   for t in CAsT.qrels_ts}

measures_orig = {r: {} for r in CAsT.runs}
for runID in CAsT.runs:
    measures_orig[runID] = {t: topic_evaluator[t].evaluate({t: CAsT.runs[runID][t]})[t][pytrec_label2] for t in CAsT.qrels_ts}
...
for c in conv_annotations:
    ...
    convs[idx] = {'root': roots, 'parents': parents}
...
for alpha in alphas:
    ...
    for runID in CAsT.runs:
        new_measures[runID] = {}
        for conv in convs:
            new_measures[runID][conv] = conv_measures.recursive_inverse_measure(convs[conv], measures[runID][conv], alpha=alpha)
...
results.to_csv(f"../data/{measure}_forward.csv")
